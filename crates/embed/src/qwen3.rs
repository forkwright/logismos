...
use logismos_core::{
    ComputeSnafu as CoreComputeSnafu, EmbeddingError, EmbeddingModel, EncodeOpts, Prompt,
    UnsupportedDimSnafu as CoreUnsupportedDimSnafu,
};
use tokenize::VerifiedTokenizer;

use crate::error::{
    InputTooLongSnafu, MetadataSnafu, NonNormalizableSnafu, Result, UnresolvedPromptRoleSnafu,
};
...
pub struct Qwen3EmbeddingLimits {
    /// Maximum UTF-8 request text bytes before prefix construction.
...
impl<'artifact> Qwen3EmbeddingModel<'artifact> {
    ...
    pub fn from_verified_cpu(
        ...
        let expected = values.iter().map(|value| match value {
            MetaValue::String(value) => value.as_str(),
            _ => "",
        });
        tokenizer
            .verify_exact_vocabulary(values.len(), expected)
            .map_err(|source| crate::error::Error::Tokenize { source })?;
        let add_bos = flag(metadata, ADD_BOS)?;
        let add_eos = flag(metadata, ADD_EOS)?;
        ...
        let weights = Qwen3Weights::try_from_verified(artifact)
            .map_err(|source| crate::error::Error::Decoders { source })?;
        ...
        Ok(Self {
            ...
            tokenizer,
            hidden,
            max_tokens: limits.max_tokens.min(max_context),
            prefixes,
            bos,
            eos,
            add_bos,
    ...
    pub fn encode_cpu(&self, text: &str, opts: &EncodeOpts) -> Result<Vec<f32>> {
        ...
        let text = prefix(&self.prefixes, opts.prompt.as_ref(), text)?;
        let mut ids = self
            .tokenizer
            .tokenizer()
            .encode(&text, false)
...
impl EmbeddingModel for Qwen3EmbeddingModel<'_> {
    ...
    fn encode(
        ...
        self.encode_cpu(text, opts).map_err(|error| {
            CoreComputeSnafu {
...
fn flag(metadata: &std::collections::HashMap<String, MetaValue>, key: &str) -> Result<bool> {
    Ok(matches!(metadata.get(key), Some(MetaValue::Bool(true))))
}
fn id(
...
fn prefix(prefixes: &Qwen3RolePrefixes, prompt: Option<&Prompt>, text: &str) -> Result<String> {
    match prompt {
        ...
        Some(Prompt::Custom(prefix)) => Ok(format!("{prefix}{text}")),
        ...
        Some(_) => UnresolvedPromptRoleSnafu { role: "unknown" }.fail(),
    }
}
fn normalize(mut values: Vec<f32>) -> Result<Vec<f32>> {
    let norm = values.iter().map(|value| value * value).sum::<f32>().sqrt();
    if !norm.is_finite() || norm == 0.0 || values.iter().any(|value| !value.is_finite()) {
