...
extern "C" __global__ void logismos_f32_row_gemv_f32_kernel(
    const std::uint8_t*, const float*, float*, int, int, int, std::uint32_t*);
...
extern "C" hipError_t logismos_launch_##NAME##_row_gemv_f32_checked( \
    const void* matrix, const void* activations, void* output, int rows, int width, int tokens, \
    void* numerical_status, hipStream_t stream) \
{ \
    const auto work_items = static_cast<unsigned long long>(rows) * static_cast<unsigned long long>(tokens); \
    const auto grid_x = static_cast<unsigned int>((work_items + THREADS_PER_BLOCK - 1U) / THREADS_PER_BLOCK); \
    const dim3 block(THREADS_PER_BLOCK, 1, 1); \
    const dim3 grid(grid_x, 1, 1); \
    /* SAFETY: Rust validates exact spans; its caller retains them plus any status through completion. */ \
    logismos_##NAME##_row_gemv_f32_kernel<<<grid, block, 0, stream>>>( \
        ...
        reinterpret_cast<float*>(output), rows, width, tokens, \
