...
extern "C" __global__ void logismos_f32_row_gemv_f32_kernel(
    const std::uint8_t*, const float*, float*, int, int, std::uint32_t*);
...
extern "C" hipError_t logismos_launch_##NAME##_row_gemv_f32_checked( \
    const void* matrix, const void* activations, void* output, int rows, int width, \
...
{ \
    ...
    /* SAFETY: Rust validates exact spans; its caller retains them plus status through completion. */ \
...
{ \
    return logismos_launch_##NAME##_row_gemv_f32_checked( \
        matrix, activations, output, rows, width, nullptr, stream); \
}
