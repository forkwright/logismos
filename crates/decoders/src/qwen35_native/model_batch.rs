...
use super::{Qwen35NativeExecutionBatchDeviceDemand, Qwen35NativeExecutionDeviceDemand, Qwen35NativeExecutionSession};
use crate::error::{ExecutionAllocationSnafu, NativeKernelSnafu, NativeSessionStateSnafu};
use crate::qwen35_native::{InFlight, PostCompletion};
use crate::Result;
...
impl Qwen35NativeExecutionSession {
    ...
    pub fn plan_batch<'session, 'tokens>(
        ...
        if sessions.is_empty() || sessions.len() != token_ids.len() {
            return NativeSessionStateSnafu {
                rule: "native model batch requires equal nonempty session and token inputs",
            }
            .fail();
        }
        let first = sessions[0].owner.ready_resource().map_err(begin_error)?;
        ...
        for (session, tokens) in sessions.iter().zip(token_ids) {
            ...
            demands.push(Qwen35NativeExecutionDeviceDemand::from_bytes(resource.device_bytes())?);
            max_context = max_context.max(resource.max_context());
        }
        let packed = PackedPrefillPlan::new(&lengths, &offsets, max_context).context(NativeKernelSnafu)?;
        ...
        for (sequence, (session, tokens)) in sessions.iter().zip(token_ids).enumerate() {
            ...
            chunks.push(ModelChunkPlan::from_packed_sequence(
                resource.plan_for_batch(), tokens, &packed, sequence,
            )?);
        }
        Ok(Self { sessions, chunks, packed, demand, token_ids })
    }
}
...
impl Qwen35NativeExecutionBatchPlan<'_, '_> {
    ...
    #[must_use]
    pub const fn device_demand(&self) -> Qwen35NativeExecutionBatchDeviceDemand { self.demand }

    /// Return the sole aggregate sequence-major geometry for this transaction.
    #[must_use]
    pub const fn packed_prefill_plan(&self) -> &PackedPrefillPlan { &self.packed }
    ...
    pub unsafe fn execute(self) -> Result<Vec<DeviceBuffer<f32>>> {
        let Self { sessions, chunks, packed: _, demand: _, token_ids } = self;
        ...
        for ((session, chunk), _tokens) in sessions.iter_mut().zip(chunks).zip(token_ids) {
            ...
            if let Err(error) = in_flight.resource().map_err(completion_error)?.prepare_chunk(chunk) {
                let requires_teardown = in_flight.resource().map_err(completion_error)?.requires_teardown();
                if requires_teardown { in_flight.poison_known_idle(); }
                return Err(error);
            }
            in_flight.mark_submitted();
            // SAFETY: the batch caller establishes every qualified session contract.
            unsafe { in_flight.resource().map_err(completion_error)?.submit_step()? };
            pending.push(in_flight);
        }
        for in_flight in pending {
            completed.push(in_flight.complete_prepublication().map_err(completion_error)?);
        }
        ...
        for prepared_cache in prepared {
            if let Some(prepared_cache) = prepared_cache { prepared_cache.commit(); }
        }
...
fn restore_outputs(completed: &mut [PostCompletion<'_, ModelSessionResources>], outputs: Vec<DeviceBuffer<f32>>) {
...
fn reserve<T>(target: &'static str, length: usize) -> Result<Vec<T>> {
    ...
    values.try_reserve_exact(length).context(ExecutionAllocationSnafu { target, length })?;
    Ok(values)
}
